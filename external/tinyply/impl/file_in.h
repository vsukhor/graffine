...
struct FileIn {
    ...
    std::size_t read_property_binary(
    ...
    std::size_t read_property_ascii(
        Type t,
        const std::size_t stride,
        void* dest,
        std::size_t& destOffset,
           ...
           const bool firstPass)
{
    ...
    for (std::size_t element_idx {};
        ...
        for (std::size_t count {}; count < element.size; ++count) {

            for (std::size_t property_idx {};
...
size_t FileIn::
read_property_binary(const std::size_t stride,
                     void* dest,
                     std::size_t& destOffset,
                     const std::size_t destSize,
                     std::istream& is)
        ...
        for (auto& [_, helper]: header.userData.get()) {
            if (helper.data == d && d->buffer.get() == nullptr) {
                ...
                if (list_hints == 0)
                    ...
                    const std::size_t list_size_multiplier =
